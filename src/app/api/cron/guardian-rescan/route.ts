...
async function findCategoryFlips(windowMinutes: number = 5): Promise<Array<{ slug: string; oldLevel: string; newLevel: string }>> {
  ...
  const { data: opportunities, error } = await (supabase as unknown)
    .from('opportunities')
    .select(`
      id,
      slug,
...
async function purgeCDNCache(slugs: string[]): Promise<number> {
  ...
  const { data: opportunities } = await (supabase as unknown)
    .from('opportunities')
    .select('id')
    .in('slug', slugs);

  if (opportunities && opportunities.length > 0) {
    const opportunityIds = opportunities.map((o: unknown) => o.id);
