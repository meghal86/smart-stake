/**
 * CORS Preflight Test for wallets-remove-address Edge Function
 * 
...
/**
 * Test that wallets-remove-address handles OPTIONS preflight correctly
 * Requirement 14.1: Every Edge Function SHALL handle `OPTIONS` preflight
 */
Deno.test('wallets-remove-address: handles OPTIONS preflight request', async () => {
  const preflightRequest = new Request('https://example.com/functions/v1/wallets-remove-address', {
    method: 'OPTIONS',
    headers: {
 ...
 */
Deno.test('wallets-remove-address: CORS headers include all required headers', () => {
 ...
 */
Deno.test('wallets-remove-address: CORS headers include allowed methods', () => {
 ...
 */
Deno.test('wallets-remove-address: preflight succeeds without Authorization header', () => {
  const preflightRequest = new Request('https://example.com/functions/v1/wallets-remove-address', {
    method: 'OPTIONS',
    headers: {
      'Origin': 'https://example.com',
 ...
 */
Deno.test('wallets-remove-address: browser calls have proper CORS headers', () => {
  const response = new Response(JSON.stringify({
    success: true,
    deleted_count: 2,
    new_primary_id: 'test-id',
  }), {
    status: 200,
    headers: {
