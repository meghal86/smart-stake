...
create policy "opps public read" on public.opportunities for select using (true);

alter table public.user_opportunities enable row level security;
create policy "uopp owner all" on public.user_opportunities
for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

alter table public.eligibility_cache enable row level security;
create policy "elig public read" on public.eligibility_cache for select using (true);
create policy "elig service insert" on public.eligibility_cache for insert with check (true);
