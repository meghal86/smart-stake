-- Full Quant Market Hub - Live Data Only (No Mock Data)

-- 1. Chain Features (24h rolling window) - Real data from whale_transfers
...
-- 4. Alert Classification Rules - Configuration only (no sample data)
...
WHERE wt.timestamp >= NOW() - INTERVAL '7 days'
  AND wt.amount_usd IS NOT NULL
  AND wt.amount_usd != 0
GROUP BY wt.chain, DATE_TRUNC('hour', wt.timestamp)
HAVING COUNT(*) > 0;
...
CREATE INDEX IF NOT EXISTS idx_whale_transfers_timestamp_chain ON whale_transfers(timestamp DESC, chain) WHERE amount_usd IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_whale_transfers_amount_usd ON whale_transfers(amount_usd DESC) WHERE amount_usd IS NOT NULL;
